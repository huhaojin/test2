...
class RGB_Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        clear = np.asarray(Image.fromarray(clear).resize((self.input_w, self.input_h)), dtype=np.float32).transpose(
            (2, 0, 1)) / 255
        nir_name = id + '_thermal_foggy_0.5.png'
...
#         return len(self.haze_imgs)
class val_Dataset(data.Dataset):
    def __init__(self, path, train, size=crop_size, input_h=1080, input_w=1920, format='.png'):  # 1920/4;1280/2
        # path = D:\dataset\FusionDataset\Airsim-VID\foggy_0.5\train\
        super(val_Dataset, self).__init__()
        self.size = size
        print('crop size', size)
        self.train = train
...
path = r'G:\FusionForImageDehze\foggy_0.5\foggy_0.5'           # path to your 'dataset' folder
...
class DehazeDataset(Dataset):
    def __init__(self, root, mode='train'):
        self.root = root
        self.mode = mode
        hazy_path = os.path.join(root, mode, 'hazy_vis')
        ir_path   = os.path.join(root, mode, 'hazy_ir')
        clear_path = os.path.join(root, mode, 'clear')

        self.names = sorted([f for f in os.listdir(hazy_path) if f.endswith('.png')])

        self.hazy_vis_paths = [os.path.join(hazy_path, n) for n in self.names]
        self.hazy_ir_paths  = [os.path.join(ir_path, n) for n in self.names]
        self.clear_paths    = [os.path.join(clear_path, n) for n in self.names]

    def __len__(self):
        return len(self.names)

    def __getitem__(self, idx):
        hazy_vis = transform(Image.open(self.hazy_vis_paths[idx]).convert('RGB'))
        hazy_ir  = transform(Image.open(self.hazy_ir_paths[idx]).convert('RGB'))   # 强制转RGB
        clear    = transform(Image.open(self.clear_paths[idx]).convert('RGB'))

        return hazy_vis, hazy_ir, clear'''